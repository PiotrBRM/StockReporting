...
export default function StockConsolidator() {
  ...
  const [excludeHighStock] = useState(true); // keep your current behavior
  ...
  const processData = () => {
    ...
    try {
      ...
      for (const row of source1Data) {
        ...
        // NOTE: "Inv Avail" is NOT a quantity — in AMPED's Inventory Analysis
        // report it's a computed "Projected Weeks of Inventory" figure
        // (QAV ÷ Avg/Week), so it must never be used as a stock fallback.
        ...
        const ampedCurWeek = ampedMatch
          ? getNumericValue(ampedMatch.CurWeek || ampedMatch["Cur Week"] || ampedMatch["Sales Week"] || 0)
          : 0;
        ...
        // AMPED's "Inv Avail" column is now a computed weeks-of-supply average
        // (QAV ÷ Avg/Week), not a quantity, so use QOH for the on-hand figure
        // this field is meant to represent.
        const ampedInvAvail = ampedMatch
          ? getNumericValue(
              ampedMatch.QOH ||
                ampedMatch.InvAvail ||
                ampedMatch["Inventory Available"] ||
                ampedMatch["Inventory Qty Available"] ||
                0
            )
          : 0;

        const ampedSales = ampedMatch
          ? getNumericValue(ampedMatch.Sales || ampedMatch.sales || ampedMatch["Total Sales"] || ampedMatch["Units Sold"] || 0)
          : 0;

        // Only exclude if BOTH Proper & AMPED are high AND there is an AMPED match
        if (excludeHighStock && ampedMatch) {
          const properTooHigh = stockOnHand > FIXED_STOCK_THRESHOLD;
          const ampedTooHigh = ampedStock > FIXED_STOCK_THRESHOLD;
          if (properTooHigh && ampedTooHigh) continue;
        }
        ...
        const avg = Math.round(((last3[0] + last3[1] + last3[2]) / 3) * 100) / 100;
        ...
        consolidated.push({
          ...
          "Pr. Sales Last Month": getNumericValue(row.Sales_LastMonth),
          "Pr. Sales 2 Months Ago": getNumericValue(row.Sales_2MonthsAgo),
          "Pr. Sales 3 Months Ago": getNumericValue(row.Sales_3MonthsAgo),
          "Pr. Avg Sales (3M)": avg,
          "AMPED Cur Week": ampedCurWeek,
          "AMPED Avg/Week": ampedAvgWeek,
          "AMPED Inv Avail": ampedInvAvail,
          "AMPED Sales": ampedSales,
          "Proper Stock": stockOnHand,
          "AMPED Stock": ampedStock,
          // Signed difference: Proper - AMPED (negative means AMPED has more)
          ...
          >
            <div>
              ...
              <h1 className="text-4xl font-thin tracking-tight leading-tight text-white">
                Proper — AMPED — Stock Consolidator
              </h1>
              <p className="text-sm font-light text-white/80 mt-1 leading-snug">
                Automatically consolidate stock reports from Proper and AMPED.
              </p>
            </div>
          </motion.header>
            ...
            <section className="grid md:grid-cols-2 gap-6">
              ...
              <motion.div
                ...
                className={`rounded-xl p-5 border-2 border-dashed cursor-pointer transition-colors ${
                  source1Data.length ? "border-indigo-400" : "border-slate-200"
                } ${dragOverTarget === 1 ? "ring-4 ring-indigo-200" : ""} hover:border-slate-400`}
                onDragOver={(e) => {
                  e.preventDefault();
                  setDragOverTarget(1);
                }}
              ...
              >
                <label htmlFor="file1" className="flex items-center gap-4 cursor-pointer">
                  <div className="relative p-3 rounded-md bg-white shadow-sm">
                    <Upload
                      className={`w-7 h-7 ${
                        source1Data.length ? "text-indigo-600" : "text-slate-400"
                      }`}
                    />
                    <AnimatePresence>
              ...
              <motion.div
                ...
                transition={{ duration: 0.15 }}
                className={`rounded-xl p-5 border-2 border-dashed cursor-pointer transition-colors ${
                  source2Data.length ? "border-indigo-400" : "border-slate-200"
                } ${dragOverTarget === 2 ? "ring-4 ring-indigo-200" : ""} hover:border-slate-400`}
                onDragOver={(e) => {
                  e.preventDefault();
                  setDragOverTarget(2);
                }}
              ...
              >
                <label htmlFor="file2" className="flex items-center gap-4 cursor-pointer">
                  <div className="relative p-3 rounded-md bg-white shadow-sm">
                    <Upload
                      className={`w-7 h-7 ${
                        source2Data.length ? "text-indigo-600" : "text-slate-400"
                      }`}
                    />
                    <AnimatePresence>
              ...
              <div className="flex items-center gap-3 w-full md:w-2/3">
                <div className="relative flex-1 hidden md:block">
                  <input
                    ...
                    className="w-full py-3 px-4 rounded-lg border border-slate-200 focus:ring-2 focus:ring-indigo-200"
                  />
                ...
                <div className="flex items-center gap-2">
                  <motion.button
                    ...
                    className="inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-gradient-to-r from-indigo-600 to-pink-600 text-white shadow-md disabled:opacity-60"
                  >
                    {processing ? (
